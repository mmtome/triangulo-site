...
/* ============================================================================
   LaserFlow — componente do React Bits (feixe volumétrico em GLSL, via three).

   Integrado sem tocar no shader: o componente já expõe `color`, e o feixe é
   monocromático por construção — passar o Vermelho Triângulo já o mantém
   dentro da paleta do Manual da Marca, sem adaptação.
   ========================================================================== */
...
export function LaserFlow({
  ...
  useEffect(() => {
    ...
    const alvoMouse = new THREE.Vector2(0, 0);
    const suaveMouse = new THREE.Vector2(0, 0);

    const dimensionar = () => {
      const w = mount.clientWidth || 1;
      const h = mount.clientHeight || 1;
      renderer.setSize(w, h, false);
      uniforms.iResolution.value.set(w * ratio, h * ratio, ratio);
    ...
    const io = new IntersectionObserver(([e]) => (naTela = e?.isIntersecting ?? true), {
      threshold: 0,
    });
    io.observe(mount);

    const onVis = () => (pausado = document.hidden);
    document.addEventListener("visibilitychange", onVis, { passive: true });

    // O container é pointer-events:none (é fundo), então o ponteiro vem da
    // janela — senão o feixe nunca reagiria ao mouse.
    const onMove = (e: PointerEvent) => {
      if (!rect) return;
      const x = e.clientX - rect.left;
      const y = e.clientY - rect.top;
      alvoMouse.set(x * ratio, rect.height * ratio - y * ratio);
    };
    ...
    const animar = () => {
      ...
      if (pausado || !naTela) return;

      const t = clock.getElapsedTime();
      const dt = Math.min(0.033, Math.max(0.001, t - prev));
      prev = t;
      ...
      suaveMouse.lerp(alvoMouse, 1 - Math.exp(-dt / tau));
      uniforms.iMouse.value.set(suaveMouse.x, suaveMouse.y, 0, 0);
  ...
  useEffect(() => {
    ...
    color, wispDensity, mouseTiltStrength, horizontalBeamOffset, verticalBeamOffset,
    flowSpeed, verticalSizing, horizontalSizing, fogIntensity, fogScale, wispSpeed,
    wispIntensity, flowStrength, decay, falloffStart, fogFallSpeed,
  ]);
